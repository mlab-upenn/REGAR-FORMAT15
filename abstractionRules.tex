...
An application example of Rule 7 is shown in \figref{rule5}. $H_{vt}'''=N^1_AP^1N^1_V$ is replaced by $H_{all}=N^2_AN^2_V$. Here we prove that for all transitions in $H_{vt}'''$, there exists a corresponding transition in $H_{all}$ so that $H_{all}$ is an over-approximation of $H_{vt}'''$. 
