...
Consider \figref{rule5} showing an application of R7, $H_{vt}'''=N^1_AP^1N^1_V$ is abstracted to $H_{all}=N^2_AN^2_V$. Here we prove that for all observable transitions in $H_{vt}'''$, there exists a corresponding transition in $H_{all}$ so that $H_{all}$ is an over-approximation of $H_{vt}'''$. 
...
$$(Rest,Idle,Rest)\xrightarrow[N^1_A.Act\_path!]{N^1_A.t\in [N^1_A.Trest\_min, N^1_A.Trest\_max] }(Rest,Ante,Rest)$$
are mapped to the following transition in $H_{all}$:
$$(Rest,Rest)\xrightarrow[N^2_A.Act\_path!]{N^2_A.t\in [0, N^1_A.Trest\_max]}(Rest,Rest)$$
Self-activations of the ventricular node:
$$(Rest,Idle,Rest)\xrightarrow[N^1_V.Act\_path!]{N^1_V.t\in [N^1_V.Trest\_min, N^1_V.Trest\_max] }(Rest,Retro,Rest)$$
are mapped to the following transition in $H_{all}$:
$$(Rest,Rest)\xrightarrow[N^2_V.Act\_path!]{N^2_V.t\in [0, N^1_V.Trest\_max]}(Rest,Rest)$$
...
are mapped to the following transition in $H_{all}$:
$$(Rest,Rest)\xrightarrow[N^2_V.Act\_path!]{N^2_V.t\in [0, N^2_V.Trest\_max]}(Rest,Rest)$$
...
are mapped to the following transition in $H_{all}$:
$$(Rest,Rest)\xrightarrow[N^2_A.Act\_path!]{N^2_A.t\in [0, N^2_A.Trest\_max]}(Rest,Rest)$$
