...
If the interval between self-activations of the atria is between 300ms to 1000ms, the interval between ventricular paces should be no shorter than 500ms.
This can be formalized using the monitor  $M_{sing}(VP,500,\infty)$ to:
...
$CE_{at}$ corresponds to fast intrinsic heart rate (i.e. when running). The pacemaker is pacing the ventricle after $TAVI$ period \hatodo{is it TAVI or AVI like in the previous seciton?}
