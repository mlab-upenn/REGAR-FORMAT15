...
To verify the closed-loop system with pacemaker model $PM$ and heart model abstraction tree $T_{HM}$ (\figref{HM_abs}) against requirement $Req1$, we start by searching for the most abstract appropriate models from the abstraction tree. The single event monitor $M_{sing}$ from \figref{monitor}.a with variables $Var(M_{sing})=\{M_{sing}.t,M_{sing}.loc\}$ is used for this requirement. The variables in the requirement are:
...
The abstraction tree is then further explored and the most refined heart models with counter-examples are: $H_{n};H_{pvc};H_{af};H_{avn};H_{afib}$.
...
\subsection{Analysis of the Counter-examples}
The counter-examples are then sent to physicians for analysis. In \figref{CE} we demonstrate 3 counter-examples from the case study. In the counter-examples, the first%We only show the activations of the atrial node and ventricle pacing. 

$CE_{n}$ corresponds to fast intrinsic heart rate. The pacemaker is pacing the ventricle after $AVI$ period %\hatodo{is it TAVI or AVI like in the previous seciton?}
...
In $CE_{af}$ the atrial rate is very high, which is a sub-optimal heart condition. \hatodo{use a word other than sub-optimal. E.g., dangerous, harmful..}
