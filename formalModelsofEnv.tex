...
In \cite{iccps10} we developed general monitors in Stateflow for closed-loop testing of physiological requirements. They can be translated to timed-automata which are shown in \figref{monitor}. The $M_{sing}(event,thresh\_min, thresh\_max)$ enforces the time interval between two $event$ signals within $[thresh\_min,thresh\_max]$. \\
