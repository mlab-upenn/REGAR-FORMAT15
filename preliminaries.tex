...
\hatodoin{HA: removed non-surjective because it's not necessary for over-approximation. See Fig.1 in Clarke's JACM paper: his function is surjective, but still introduces over-approximation.}
...
We represent the over-approximation relationship as $M\triangleleft_h M'$, and the relationship between reachable behavior spaces as $\absfun(\mathbb{B}(M))\subseteq\mathbb{B}(M')$. 

\hatodoin{Let $\varphi$ be a requirement on $M$ expressed in ACTL$^*$. 
	The abstraction function is \emph{appropriate} for the requirement $\varphi$ if 
	$\absfun(M) \models \varphi \implies M \models \varphi$
	}

With a requirement $\varphi$ in $ACTCL^*$\hatodo{what's this logic? give a reference}, we have:
...
We denote it as $\{M_1,M_2\}\triangleleft_h M'$, such that $\absfun(\mathbb{B}(M_1)\cup\mathbb{B}(M_2))\subseteq\mathbb{B}(M')$.
...
Behaviors introduced into the over-approximation of a system \hatodo{device?} model are all \emph{spurious}, or invalid.\hatodo{stick to one word, either spurious or invalid} 
...
\hatodo{Why make this argument here? This is Technical Preliminaries, where we just define a few things so we can go on with the paper...}
%\begin{figure}[!b]
%		\centering
