...
param Px0 {VERTICES} >= 0;
# Terms for the linear predicate Phi
param C {RULES};
param B {RULES, VERTICES};
# Preserved vertices
...
set KX within {SUBGRAPHS, VERTICES};
...
#set FUCKED  = {(i,x) in RULES cross VERTICES: 
...
set mySUBGRAPHS {x in VERTICES} = {s in SUBGRAPHS : (s,x) in KX};
#var Px {(x,j) in VERTICES cross 1..N} = Px[x,j] + sum {(i,s) in KRULES: s in mySUBGRAPHS[x]}  a[i,s,j-1]*(-Px[x,j] + max( Px[x,j], Px[x, j-1]));
var Px {(x,j) in VERTICES cross TIME0} = if j = 0 then Px0[x] else Px[x,j-1] + sum {(i,s) in KRULES: s in mySUBGRAPHS[x]}  a[i,s,j-1]*(-Px[x,j-1] + max( 430, Px[x, j-1]));
...
set KX2{s in SUBGRAPHS} = {x in VERTICES : (s,x) in KX};
